...
from src.services.order_manager import OrderManager, get_order_manager
...
async def create_order(product_id: UUID,
    ...
    if not (user := await db_manager.async_get_by_id(User, user_id)):
        user = User(customer_id=stripe_manager.create_customer().stripe_id)
    ...
    product = await db_manager.async_get_by_id(Product, product_id)
    order = await order_manager.async_build_and_create(user, product)

    # TODO: передать на оплату
